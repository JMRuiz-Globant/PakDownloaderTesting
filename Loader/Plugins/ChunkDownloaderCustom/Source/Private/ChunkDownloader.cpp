...
public:
	...
	void DoWork()
	{
		...
		if (FCoreDelegates::MountPak.IsBound())
		{
			...
			for (const TSharedRef<FPakFileRecord>& PakFile : PakFiles)
			{
				...
				if (MountedPak)
				{
					...
					MountedPakFiles.Add(PakFile, Result);
					--PakReadOrder;
				}
				else
				{
...
public: // results
	...
	TMap<TSharedRef<FPakFileRecord>, FPakMountWorkResult> MountedPakFiles;
};
...
void FChunkDownloaderCustom::CompleteMountTask(FChunk& Chunk)
{
	...
	for (const auto& MountWorkResult : MountWork.MountedPakFiles)
	{	
