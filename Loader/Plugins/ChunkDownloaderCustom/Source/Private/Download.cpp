...
bool FDownloadChunk::ValidateFile() const
{
	...
	{
		UE_LOG(LogChunkDownloaderCustom, Error, TEXT("Size mismatch. Expected %llu, got %llu"), PakFile->Entry.FileSize, PakFile->SizeOnDisk);
		return false;
	}
...
bool FDownloadChunk::HasDeviceSpaceRequired() const
{
	...
	if (FPlatformMisc::GetDiskTotalAndFreeSpace(Downloader->CacheFolder, TotalDiskSpace, TotalDiskFreeSpace))
	{
		...
		{
			// not enough space
			UE_LOG(LogChunkDownloaderCustom, Warning, TEXT("Unable to download '%s'. Needed %llu bytes had %llu bytes free (of %llu bytes)"),
...
void FDownloadChunk::OnDownloadComplete(const FString& Url, int TryNumber, int32 HttpStatus)
{
	...
	if (EHttpResponseCodes::IsOk(HttpStatus))
	{
		...
		UE_LOG(LogChunkDownloaderCustom, Error, TEXT("%s from %s failed validation"), *TargetFile, *Url);
